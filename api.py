...
class GET_PAGE_SIZE(Resource):
  def get(self,URL):
    try:
      ...
      #example['result'] = keys 
      driver.quit()
      return {'counter':str(counter), 'bodySize':str(sum(bodySize)), 'time':str(sum(download_time)), 'startDownloadTime':str(proxy.har['log']['entries'][0]['startedDateTime']),'har':{0:proxy.har}}
